...
// react-router-dom이란?
// 리액트를 위한 라우팅 라이브러리
// 라이팅: 사용자가 원하는 경로로 보내는 과정

// 페이지를 나누는 법

// 1) 리액트 미사용 시
// MPA라서 각 페이지 별 html 문서를 만들어야 함
// /list 경로로 접속한다면 해당 list.html 파일을 보내줌

// 2) 리액트 사용 시
// SPA라서 index.html 파일 하나만 존재
// List 컴포넌트를 만들고
// /list 경로로 접속한다면 기존 컴포넌트 자리에 List 컴포넌트를 보여줌
		...
		<BrowserRouter>
			<Routes>
				...
				{/* URL 파라미터는 주소의 경로에 유동적인 값을 넣는 형태고,
					쿼리스트링은 주소의 맨 뒷부분에 ? 문자열 이후에 key=value 형태로 값을 정의하며 &로 구분하는 형태 */}
				
				{/* 주로 URL 파라미터는 ID 또는 이름을 사용하여 특정 데이터를 조회할 때 사용하고,
					쿼리스트링은 키워드 검색, 페이지네이션, 정렬 방식 등 데이터 조회에 필요한 옵션을 전달할 때 사용 
					(꼭 위와 같이 쓰라고 정해진 규칙이 있는 것은 아님) 
					
					URL 파라미터 예시: https://vibe.naver.com/today
					쿼리 스트링 예시: 
				*/}
