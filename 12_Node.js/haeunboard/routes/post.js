...
// 발급받은 액세스 키랑 비밀키 기입(털리면 안되니까 .env에 저장 후 불러와서 사용)
...
// 버킷이름 설정
// 저장할 파일명도 바꿀 수 있음
// 파일명을 안 겹치게 하려면 랜덤한 문자(uuid)를 넣거나 현재 시간(timestamp)을 섞거나
// 이렇게 하는 이유? 파일 이름이 중복되면 덮어씌우기 때문에!
const upload = multer({
  storage: multerS3({
    s3,
    bucket: 'haeunboard', // 만든 버킷 이름
...
// 여기까지 세팅되면 upload.single('input의 name') 미들웨어 사용으로 S3에 업로드 가능
  ...
  try {
    ...
    if (!title) { 
      ...
      await db.collection('post').insertOne({ 
        ...
        imgUrl: req.file?.location || '', // 이미지 URL을 글과 함께 DB에 저장(옵셔널 체이닝 사용해도됨)
        // 글 등록 시 작성자 정보 넣기
        user: req.user._id,
        username: req.user.username,
        // username(수정 가능한 정보라고 가정) 넣었을 때 문제점: 
        // 관계형DB: 사용자의 _id만 적어두고 JOIN을 써서 사용자의 정보를 가져와 합침
        // 비관계형DB: 그냥 사용자 정보를 그대로 넣는게 관습!, 장점은 다른 컬랙션을 찾아볼 필요 없음
        // 단점은 바뀐 정보를 전부 찾아서 업데이트 하거나 업데이트가 안됐으면 정보가 부정확 할 수 있음

        // (비관계형 DB의 경우)개발자 선택 사항임
        // 1. DB 입출력 속도 Up, 데이터의 정확도 Down = > 바뀔 수 있는 정보도 같이 저장
        // 2. DB 입출력 속도 Down, 데이터의 정확도 Up = > _id 값만 저장(추천)
        // 2번을 선택하면 그 안에서도 선택지가 다양함
        // 1) findOne을 2번 쓰던가(글도 가져오고, 사용자도 가져오고)
        // 2) 몽구스의 populate, 몽고디비의 aggregate dustkswk wnd $lookup
      });
    
      // 동기식 요청이면 다른 페이지로 이동
      // res.redirect('/post');

      // 비동기(Ajax) 요청이면 성공데이터와 함께 응답
      // 응답으로 redirect와 render,sendFile은 사용 안하는게 좋음
      res.json({
        flag: true, 
        message: '등록 성공'
      });
...
router.get('/detail/:id', async (req, res, next) => {
  ...
  // DB에서 글 가져오기
  // 테스트
  // const post = await db.collection('post').findOne({ _id: '656837e4753415733aff34f4' });
  // console.log(post); // _id에 문자열 쓰는건 몽구스에서만 가능
  // const post = await db.collection('post').findOne({ _id: new ObjectId('656837e4753415733aff34f4') });
  // console.log(post); // ObjectId 객체로 만듦

  // 예외 처리 하기
  // 1) url 잘못 입력 시
  // 2) 데이터를 못 찾을 때(잘못된 id) => null 반환
...
// 애를 들어 글 1000개를 전부 가져와서 보여주도록 하면 DB도 부담되고 브라우저도 부담이 됨
// 1) 페이지 이동 버튼 만들기
...
// 검색 기능 만들기
// 1) 검색 UI(input과 검색 버튼)에서 키워드 입력 시 서버로 검색어 전송
...
router.get('/search', async (req, res) => {
  ...
  // 2. 검색어와 포함된 document를 찾으려면 => 정규표현식(정규식)
  ...
  // 예: documnet가 1억개 있으면 1억개를 다 뒤져봄
  // 해결책: 데이터베이스에 index를 만들어두면 됨

  // 3. index를 사용한 검색
  // $text: text index(복사본)를 갖다 쓰겠다는 의미
  // $search: 검색 키워드
  ...
  //   {
  //     $search: { // search index를 이용해 full-text search를 수행
  //       index: 'title_index', // 사용할 index 이름
  //       text: {
  //         query: keyword, // 검색어
