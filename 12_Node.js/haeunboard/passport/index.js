...
const local = require('./localStrategy'); // 로컬 로그인 전략(=로그인 시 어떻게 처리할지 동작 작성)
...
module.exports = () => {
  ...
  passport.serializeUser((user, done) => {
    ...
    // 두번째 인자값: 세선에 저장할 데이터(일단 서버 메모리에 저장됨)
    // 로그인 시 사용자 데이터를 세션에 저장하는데 세선에 사용자 정보를 모두 저장하면
    // 세선의 용량이 커지고 데이터의 일관성에 문제가 발생하므로 아이디만 저장
  ...
  passport.deserializeUser(async (id, done) => {
    // console.log(id); // 세션에 저장한 내용 들어있음

    try {
      const user = await db.collection('user').findOne({ _id: new ObjectId(id) });
      done(null, user); // req.user에 저장
      // 이제 아무데서나 req.user를 쓰면현재 로그인한 사용자의 정보를 쓸 수 있음
    } catch (err) {
      done(err);
    }
  });
