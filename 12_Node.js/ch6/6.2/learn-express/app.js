...
// 미들웨어간 데이터 공유 및 전달하기
// 1) app.set은 서버 내내 유지(전역 속성/서버 어디서든 사용 가능)
// 2) req.session은 나에 한해서(=같은 세션 안에서) 계속 유지하고 싶은 데이터 => 로그인 유저 정보
// 3) req, res는 요청 하나 동안 유지(1회성)
...
// multer 자체가 미들웨어는 아니고 multer 
// 함수를 호출하면 나오는 객체가 있는데 그 객체 안에 4가지 미들웨어 들어있음
const upload = multer({
  storage: multer.diskStorage({ 
  // 하드디스크에 저장 (실제 서버 운영 시 서버 디스크 대신 클라우드 스토리지 서비스에 저장하는 게 좋음)
    destination(req, flie, done) { // 어디에 저장할지
      done(null, 'uploads/'); // 주의! uploads 폴더가 실제로 없으면 업로드 시 에러남 미리 생성 해야됨!(바로 위 trycatch문)
    },
    ...
    // done(에러 시 에러값, 성공 시 전달할 값); // 에러 발생 시 에러 처리 미들웨어
  }),
...
// => 인자값은 input태그의 name 속성과 일치해야함
// app.post('/upload', upload.single('image'), (req, res) => { // 라우터 전에 미들웨어 장착
...
// 3) 여러 파일(input 태그를 여러개 사용해서 name이 다른 경우)을 업로드 하는 경우
app.post('/upload', 
