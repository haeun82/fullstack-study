...
console.log('path.basename() - extname():', path.basename(string, path.extname(string))); // 파일명에서 확장자 빼는 법

console.log('---------------------------------------');
console.log('path.parse():', path.parse(string)); // 파일 경로를 각각의 요소들로 분해해서 객체로 반환
console.log('path.format():', path.format({ // 객체를 파일 경로로 다시 합치기
  dir: 'C:\\users\\zerocho',
  name: 'path',
  ext: '.js'
}));
console.log('path.normalize():', path.normalize('C://users\\\\zerocho\\\path.js')); // 구분자를 실수로 잘못 적었을 때 경로를 os에 맞춰 정상적으로 만들어줌
