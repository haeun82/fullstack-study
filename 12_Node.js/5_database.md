...
예를 들어 취미, 주문내역(객체나 배열 자료형 사용이 안됨)
이 때는 컬럼을 하나 더 만들거나, 컬럼이 너무 많아지게 될 경우 차라리 새로운 테이블로 추출함(정규화)
...
3. 비교 
두 데이터이스가 공존하는 유이는 바로 각각의 명확한 장점 때문
...
서비스의 변경사항이 잦아서 쉽고 유연하기 데이터를 저장하고 싶으면 NoSQL을 사용

근데 꼭 이렇게 분리해서 써야되는게 아니라 어떻게 활용하느냐에 따라서 다를 수 있음
...
2) 회원가입 username: cjwd829 / password: hlPVHhRO8K69GWWp
3) M0 무료 티어선택, 서버 위치도 선택
...
관리자용으로 admin/기억할 수 있는 비번으로 또는 root/기억할 수 있는 비번 만들고 잘 기억해두기 admin/admin123456
(주의) 역할을 altas admin으로 설정해야됨
그래야 Node.js에서 DB 접속 시 모든 기능을 다 사용 가능
5) 좌측 Network Access 메뉴에서 IP 추가
데이터베이스에 접속할 수 있는 IP를 미리 정의해놓는 일종의 보안장치
Allow access from anywhere르 누르거나 0.0.0.0/0 을 추가하기

7. 데이터 흐름
유저(클라이언트) <-> 서버로 데이터 요청 <-> 서버에서 DB 조회
