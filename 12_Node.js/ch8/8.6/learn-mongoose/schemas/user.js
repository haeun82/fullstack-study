...
const userSchema = new Schema({ // 스키마 정의: RDB의 테이블처럼 정해진 데이터만 들어갈 수 있게 강제함
  ...
  comment: String, // 옵션이 type 밖에 없을 땐 간단하게 생략 가능
  createdAt: {
    type: Date,
...
module.exports = mongoose.model('User', userSchema); // 'User'가 users라는 컬렉션으로 변환됨

// (정리)
// 몽구스를 이용하여 기능의 확장 및 제한을 둘 수 있음 => 마치 SQL RDB처럼 사용
// 몽고디비의 장점은 자유로운 확장성인데 스키마를 만든다?
// => JS로 DB를 다루고 싶은데 SQL처럼 쓰고 싶어서
