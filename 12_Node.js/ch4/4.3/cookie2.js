...
http.createServer(async (req, res) => {
  ...
  if (req.url.startsWith('/login')) {
    ...
    expires.setMinutes(expires.getMinutes() + 1); // 1분 뒤 쿠키 삭제됨 -> 로그인 풀림

    console.log(name); // 한글일 때 encodeURIComponent로 인코딩 안하면 쿠키에 이상한 문자가 들어갔다고 에러 발생
...
// 개발자 도구만 쓸 줄 알면 정보가 다 보이고 조작 가능
// 그래서 중요한 정보는 서버에서 들고 있고 그 정보에 접속할 수 있는 키값 (=세션 ID)만 브라우저로 보내서 쿠키에 넣는데 그게 바로 세션 방식
